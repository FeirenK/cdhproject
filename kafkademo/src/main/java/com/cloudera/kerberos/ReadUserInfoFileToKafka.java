...
public class ReadUserInfoFileToKafka {
    ...
    private static  Properties getPublicProps() {
        ...
        try{
            File file = new File(confPath + File.separator + "0285.properties");

            if(!file.exists()) {
                InputStream in = ReadUserInfoFileToKafka.class.getClassLoader().getResourceAsStream("0285.properties");
                properties.load(in);
            } else {
